...
class ServiceData:
    ...
    async def FindOpeningTimes(self):
        ...
        with open("htmlDetail.txt", "r") as f:
            ...
            for line in f:
                ...
                if not ("@" in line.strip() or '}' in line.strip()
                        or '{' in line.strip()):
    ...
    async def ShowOpeningTimes(self):
        embed = discord.Embed(title=self.name + " Opening Times Are:",
                              colour=GREEN)
        for i in range(0, len(self.openingTimes)):
            if not (':' in self.openingTimes[i]):
                embed.add_field(name=self.openingTimes[i],
                                value=(self.openingTimes[i + 1] + "\n" +
                                       self.openingTimes[i + 2]),
                                inline=False)
        return embed
    ...
    def FormatOpenMsg(self):
        if self.isServiceOpen:
            return discord.Embed(title=self.name,
                                 description=self.name + " is open\n:)",
                                 colour=GREEN)
        else:
            return discord.Embed(title=self.name,
                                 description=self.name + " is closed\n:(",
                                 colour=RED)
...
@client.command(
    description=
    "Shows the service's opening times. For example, \nEnter\"ot koofi\" to get koofi's opening times",
...
async def ot(ctx, arg):
    ...
    if arg == "koofi":
        ...
        embed = discord.Embed(
            ...
            description=
            "Only send one place as one word\nSuch as: \nstannarybar\nFalmouthArtShop",
            colour=RED)
...
@client.command(name="amata",
                aliases=[
                    "Amata", "AMATA", "academyofmusicandtheatrearts",
                    "AcademyOfMusicAndTheatreArts"
                ],
...
@client.command(name="esi",
                aliases=[
                    "ESI", "esicafe", "esiCafe", "ESICAFE",
...
async def stannaryBar(msg):
    stanBar = ServiceData("Stannary Bar",
                          data.data["FoodAndDrink"]["Stannary Bar"])
    await stanBar.SetOpenData()
    await msg.channel.send(embed=stanBar.FormatOpenMsg())


@client.command(name="stannaryBarOt",
                aliases=[
                    "stannarybarot", "stannaryBarOT", "stannaryBAROT",
                    "STANNARYBAROT", "barOt", "BAROT", "barOT"
                ],
...
@client.command(
    ...
    aliases=[
        "stannarykitchen", "stannaryKITCHEN", "STANNARYKITCHEN", "stankitchen"
    ],
...
async def stannaryKitchen(msg):
    stannaryKitchen = ServiceData(
        "Stannary Kitchen", data.data["FoodAndDrink"]["Stannary Kitchen"])
    await stannaryKitchen.SetOpenData()
    await msg.channel.send(embed=stannaryKitchen.FormatOpenMsg())


@client.command(name="stannaryKitchenOt",
                aliases=[
                    "stannarykitchenot", "stannaryKitchenOT",
                    "stannaryKITCHENOT", "STANNARYKITCHENOT", "kitchenOt",
                    "kitchenOT"
                ],
...
async def stannaryKitchenOt(msg):
    ...
    await msg.channel.send(content=None,
                           embed=stannaryKitchen.openingTimesFormatted)


@client.command(name="fox",
...
async def susCafe(msg):
    susGuy = ServiceData("The Sustainability Cafe",
                         data.data["FoodAndDrink"]["Sus cafe"])
...
@client.command(
    name="susCafeOt",
    aliases=[
        "suscafeot", "susCafeOT", "susCAFEOT", "SUSCAFEOT", "susOt", "susOT",
        "SUSOT"
    ],
...
async def penrynShop(msg):
    penrynShop = ServiceData("Penryn Campus Shop",
                             data.data["FoodAndDrink"]["Penryn Shop"])
    await penrynShop.SetOpenData()
    await msg.channel.send(embed=penrynShop.FormatOpenMsg())


@client.command(name="penrynShopOt",
                aliases=[
                    "penrynshopOT", "penrynSHOPOT", "PENRYNSHOPOT", "shopOt",
                    "shopot", "shopOT", "SHOPOT"
                ],
...
async def penrynShopOt(msg):
    ...
    await msg.channel.send(content=None,
                           embed=penrynShop.openingTimesFormatted)
...
async def falmouthShop(msg):
    falmouthShop = ServiceData(
        "Falmouth Campus Art Shop",
        data.data["Facilities and Services"]["Falmouth Art Shop"])
...
@client.command(
    name="falmouthShopOt",
    aliases=[
        "falmouthshopot", "falmouthshopOt", "falmouthshopOT", "falmouthSHOPOT",
        "FALMOUTHSHOPOT", "falot", "falOt", "FALOT"
    ],
...
async def falmouthShopOt(msg):
    ...
    await msg.channel.send(content=None,
                           embed=falmouthShop.openingTimesFormatted)
...
async def flexsiSportsCentre(msg):
    gym = ServiceData(
        "Flexsi Gym",
        data.data["Facilities and Services"]["Flexsi Sports Centre"])
    await gym.SetOpenData()
    await msg.channel.send(embed=gym.FormatOpenMsg())


@client.command(name="gymOt",
